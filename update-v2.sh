...
if [[ -e /usr/local/bin/xray ]]; then
	 ...
	 sleep 1
     mv /usr/local/bin/xray /usr/local/bin/xray.bak && wget -q -O /usr/local/bin/xray "https://github.com/KhaiVpn767/Xcore-custompath/releases/download/v1.0.0/xray.linux.64bit" && chmod 755 /usr/local/bin/xray
	 echo ""
	 ...
	 sleep 1
	 rm -rf /usr/local/sbin/helium
	 wget -q -O /usr/local/sbin/helium https://raw.githubusercontent.com/KhaiVpn767/MultiportV3/main/helium.sh
	 cd /usr/bin
	 rm nf
	 rm ins-helium
	 rm limit
