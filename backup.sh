...
cd /root
zip -r $InputPass $IP-$date-$domain-MultiportV3.zip backup > /dev/null 2>&1
rclone copy /root/$IP-$date-$domain-MultiportV3.zip dr:backup/
url=$(rclone link dr:backup/$IP-$date-$domain-MultiportV3.zip)
...
rm -rf /root/backup
rm -r /root/$IP-$date-$domain-MultiportV3.zip
echo ""
